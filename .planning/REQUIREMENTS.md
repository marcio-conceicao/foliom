...
- [x] **EDT-09**: Autocomplete for `[[page]]` and `#tag` triggers on `[[` and `#` respectively, suggesting from the indexed page/tag set.
...
- [x] **EDT-11**: User can copy/cut/paste blocks preserving their hierarchy (multi-line clipboard format with bullet indentation).
- [x] **EDT-12**: A block context menu exposes the common operations (cut, copy, duplicate, fold, zoom, copy as markdown).
...
- [x] **DSK-01**: Foliom ships a Tauri 2 desktop binary that wraps the same Svelte UI consuming the in-process axum server via `WebviewUrl::External` — the WebView loads `http://127.0.0.1:<port>/` directly; no `tauri-plugin-localhost` crate required (that plugin runs its own `tiny_http` server, not a bridge to axum).
- [x] **DSK-02**: macOS and Windows installers are code-signed; macOS is notarized.
- [x] **DSK-03**: Desktop binary RSS + disk footprint is materially smaller than an Electron equivalent (target: < 30 MB installer, < 150 MB RSS at idle).
